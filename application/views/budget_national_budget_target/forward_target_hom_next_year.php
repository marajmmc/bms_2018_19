...
<form id="save_form" action="<?php echo site_url($CI->controller_url.'/index/save_forward_target_hom_next_year');?>" method="post">
    ...
    <div class="row widget">
        ...
        <div class="row show-grid">
            <div class="col-xs-4">
                <label class="control-label pull-right">Forward HOM Target Next 3 Years<span style="color:#FF0000">*</span></label>
            </div>
            <div class="col-sm-4 col-xs-8">
                <select class="form-control" name="item[status_forward_hom_target_next_year]">
...
<script type="text/javascript">
    $(document).ready(function ()
    {
        ...
        var url = "<?php echo site_url($CI->controller_url.'/get_items_forward_target_hom_next_year');?>";
        ...
        $("#system_jqx_container").jqxGrid(
            {
                ...
                columns:
                    [
                        ...
                        {
                            ++$serial;
                            ?>{columngroup: 'budget_next_years',text: '<?php echo $budget['name']; ?>', dataField: 'quantity_prediction_<?php echo $serial; ?>',width:'100',filterable: false,cellsrenderer: cellsrenderer,align:'center',cellsAlign:'right',editable:false,aggregates: [{ 'total':aggregates}],aggregatesrenderer:aggregatesrenderer},
                        <?php
                        }
                        $serial=0;
                        foreach ($fiscal_years_next_budgets as $budget)
                        {
                            ++$serial;
                            ?>{columngroup: 'target_next_years',text: '<?php echo $budget['name']; ?>', dataField: 'quantity_target_hom_<?php echo $serial; ?>',width:'100',filterable: false,cellsrenderer: cellsrenderer,align:'center',cellsAlign:'right',editable:false,aggregates: [{ 'total':aggregates}],aggregatesrenderer:aggregatesrenderer},
                        <?php
                        }
                        ?>
                    ],
