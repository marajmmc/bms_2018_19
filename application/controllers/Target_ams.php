...
class Target_ams extends Root_Controller
{
    ...
    private function system_edit($id)
    {
        if (isset($this->permissions['action2']) && ($this->permissions['action2'] == 1)) {
            ...
            $results = Query_helper::get_info($this->config->item('table_bms_target_tsme'), array('territory_id', 'amount_target'), array('ams_id =' . $item_id, 'status ="' . $this->config->item('system_status_active') . '"'));
            foreach ($results as $result) {
    ...
    private function system_save()
    {
        ...
        if ($result_exist) // EDIT
        {
            foreach ($amount_target as $location_id => $amount) {
                ...
                Query_helper::update($this->config->item('table_bms_target_tsme'), $items, array('ams_id =' . $item_id, 'territory_id =' . $location_id, "status ='".$this->config->item('system_status_active')."'")); // UPDATE into Details Table
            }
        } else {
            foreach ($amount_target as $location_id => $amount) {
                $items = array(
    ...
    private function system_save_delete()
    {
        ...
        Target_helper::delete_target_tree('ams');
    ...
    private function get_item_info($item_id) // Common Item Details Info
    {
        $this->common_query(); // Call Common part of below Query Stack
        // Additional Conditions -STARTS
        $this->db->join($this->config->item('table_bms_target_tsme'). ' details', "details.ams_id = target.id AND details.status ='".$this->config->item('system_status_active')."'", 'LEFT');
        $this->db->select('SUM(details.amount_target) AS amount_allocated');
