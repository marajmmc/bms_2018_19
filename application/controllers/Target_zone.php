...
class Target_zone extends Root_Controller
{
    public $message;
    public $permissions;
    ...
    private function system_list()
    {
        if (isset($this->permissions['action0']) && ($this->permissions['action0'] == 1)) {
            ...
            $data = array();
            $data['system_preference_items'] = System_helper::get_preference($user->user_id, $this->controller_url, $method, $this->get_preference_headers($method));
            $data['title'] = "Monthly " . ($this->lang->line('LABEL_ZONE_NAME')) . " Target List";
            $ajax['status'] = true;
    ...
    private function system_get_items()
    {
        ...
        // Details Table
        $this->db->from($this->config->item('table_bms_target_territory'));
        $this->db->select('target_zone_id, SUM(amount_target) AS amount_allocated');
        $this->db->where('status', $this->config->item('system_status_active'));
        $this->db->group_by('target_zone_id');
        $results = $this->db->get()->result_array();

        $detail_items = array();
        foreach ($results as $result) {
            $detail_items[$result['target_zone_id']] = $result;
        }
    ...
    private function system_list_all()
    {
        if (isset($this->permissions['action0']) && ($this->permissions['action0'] == 1)) {
            ...
            $data = array();
            $data['system_preference_items'] = System_helper::get_preference($user->user_id, $this->controller_url, $method, $this->get_preference_headers($method));
            $data['title'] = "Monthly " . ($this->lang->line('LABEL_ZONE_NAME')) . " Target All List";
            $ajax['status'] = true;
    ...
    private function system_list_deleted()
    {
        if (isset($this->permissions['action0']) && ($this->permissions['action0'] == 1)) {
            ...
            $data = array();
            $data['system_preference_items'] = System_helper::get_preference($user->user_id, $this->controller_url, $method, $this->get_preference_headers($method));
            $data['title'] = "Monthly " . ($this->lang->line('LABEL_DIVISION_NAME')) . " Target Deleted List";
            $ajax['status'] = true;
    ...
    private function system_edit($id)
    {
        if (isset($this->permissions['action2']) && ($this->permissions['action2'] == 1)) {
            ...
            $results = Query_helper::get_info($this->config->item('table_bms_target_territory'), array('territory_id', 'amount_target'), array('target_zone_id =' . $item_id, "status ='".$this->config->item('system_status_active')."'"));
    ...
    private function system_save()
    {
        ...
        Query_helper::update($this->config->item('table_bms_target_zone'), array(), array("id = " . $item_id)); // UPDATE into Main Table

        $results_old = Query_helper::get_info($this->config->item('table_bms_target_territory'), array('id'), array('target_zone_id =' . $item_id, "status ='" . $this->config->item('system_status_active') . "'"));
        if ($results_old) // EDIT
        {
            foreach ($amount_target as $location_id => $amount) {
                ...
                Query_helper::update($this->config->item('table_bms_target_territory'), $items, array('target_zone_id =' . $item_id, 'territory_id =' . $location_id, "status ='" . $this->config->item('system_status_active') . "'")); // UPDATE into Details Table
            }
        }
        else{
            foreach ($amount_target as $location_id => $amount) {
                $items = array(
                    'target_zone_id' => $item_id,
                    'year' => $item['year'],
                    'month' => $item['month'],
                ...
                Query_helper::add($this->config->item('table_bms_target_territory'), $items, FALSE); // INSERT into Details Table
            }
        }
    ...
    private function system_details_deleted($id)
    {
        if (isset($this->permissions['action0']) && ($this->permissions['action0'] == 1)) {
            ...
            $result = Query_helper::get_info($this->config->item('table_bms_target_zone'), array('year', 'month'), array('id ='.$item_id), 1);
            $params = array(
                'year' => $result['year'],
                'month' => $result['month'],
                'main_table' => $this->config->item('table_bms_target_zone'),
                'details_table' => $this->config->item('table_bms_target_territory'),
                'location_table' => $this->config->item('table_login_setup_location_territories'),
                'location_id_field' => 'territory_id',
                'foreign_key' => 'target_zone_id'
            );
            $data = Target_helper::get_delete_info($params);

            $data['title'] = ($this->lang->line('LABEL_DIVISION_NAME')) . " Deleted Target Details ( " . DateTime::createFromFormat('!m', $result['month'])->format('F') . ", {$result['year']} )";
            $ajax['status'] = true;
    ...
    private function system_delete($id)
    {
        if (isset($this->permissions['action3']) && ($this->permissions['action3'] == 1)) {
            ...
            $data['id'] = $item_id;
            $data['title'] = "Delete " . ($this->lang->line('LABEL_TERRITORY_NAME')) . "-wise Target (ID: " . $item_id . ")";
            $ajax['status'] = true;
    ...
    private function system_save_forward()
    {
        ...
        $this->common_query(); // Call Common part of below Query Stack
        // Additional Conditions -STARTS
        $this->db->join($this->config->item('table_bms_target_territory') . ' details', 'details.target_zone_id = target.id');
        $this->db->select('SUM(details.amount_target) AS amount_allocated');

        $this->db->where('target.status', $this->config->item('system_status_active'));
        $this->db->where('target.id', $item_id);

        $this->db->group_by('details.target_zone_id');
        // Additional Conditions -ENDS
        $result = $this->db->get()->row_array();
        $this->db->flush_cache(); // Flush/Clear current Query Stack
        ...
        // Main Table UPDATE
        Query_helper::update($this->config->item('table_bms_target_zone'), $item, array("id =" . $item_id), FALSE);
    ...
    private function get_item_info($item_id) // Common Item Details Info
    {
        $this->common_query(); // Call Common part of below Query Stack
        // Additional Conditions -STARTS
        $this->db->join($this->config->item('table_bms_target_territory') . ' details', "details.target_zone_id = target.id AND details.status = '".$this->config->item('system_status_active')."'", 'LEFT');
        $this->db->select('SUM(details.amount_target) AS amount_allocated');
        ...
        //---------------- Basic Info ----------------
        $user_acting = $this->lang->line('LABEL_ZONE_NAME') . " ";
        $parent_acting = $this->lang->line('LABEL_DIVISION_NAME') . " ";
        ...
        //Details Data
        $data['details_title'] = $this->lang->line('LABEL_ZONE_NAME') . ' Target Distribution';
        $location_id_field = 'territory_id';
        $foreign_key = 'target_zone_id';

        $this->db->from($this->config->item('table_bms_target_territory') . ' details');
    ...
    private function common_query()
    {
        ...
        $this->db->from($this->config->item('table_bms_target_zone') . ' target');
        $this->db->select('target.*, target.revision_count AS no_of_edit');

        $this->db->join($this->config->item('table_bms_target_division') . ' parent', 'parent.id = target.target_division_id AND parent.status_forward="' . $this->config->item('system_status_forwarded') . '"', 'INNER');
